...
INTERVAL_DAJIALA_MIN = _get_int("INTERVAL_DAJIALA_MIN", 1440)        # 大家啦 API 1 天 1 次（备用微信渠道）
INTERVAL_MINI_DIGEST_MIN = _get_int("INTERVAL_MINI_DIGEST_MIN", 720)  # Dashboard 聚合缓存 12h 一次

# 大家啦 API（dajiala.com）— WeChat 备用采集渠道。WeWeRSS 登录态会失效，
# 这一路用 API key 鉴权独立兜底两位高优博主。
DAJIALA_KEY = _get("DAJIALA_KEY")
DAJIALA_ACCOUNTS = _get("DAJIALA_ACCOUNTS", "赛博禅心,数字生命卡兹克")
